"""Per-target regression workflow."""
...
def _run_target_specific_task(
    ...
    y_pred_all = np.zeros_like(y_target, dtype=float)
    ...
    row: dict[str, object] = {
        ...
        "feature_set": fs.name,
        "model": spec.name,
        "rmse": _rmse(y_target, y_pred_all),
        "mae": float(mean_absolute_error(y_target, y_pred_all)),
        "r2": float(r2_score(y_target, y_pred_all)),
        "best_params_last_fold": json.dumps(fold_params[-1] if fold_params else {}, sort_keys=True),
    }
    return target, fs.name, spec.name, row


def run_regression_target_specific(
    x_user: pd.DataFrame,
    y_reg: pd.DataFrame,
    ...
    """Run per-target regression benchmark across feature sets and models."""
    selected_sets = feature_sets[: max_feature_sets or len(feature_sets)]
    rows: list[dict[str, object]] = []
    progress = ProgressTracker(total=max(1, len(y_reg.columns) * len(selected_sets) * len(model_specs)))
    tasks = [(target, fs, spec) for target in y_reg.columns for fs in selected_sets for spec in model_specs]

    if workers > 1:
        with ProcessPoolExecutor(max_workers=workers) as executor:
            futures = [
                executor.submit(
                    _run_target_specific_task,
                    target,
                    fs,
                    spec,
                    x_user,
                ...
                for target, fs, spec in tasks
            ]
            for future in as_completed(futures):
                target, fs_name, model_name, row = future.result()
                rows.append(row)
                elapsed, eta = progress.step()
                if logger is not None:
                    logger.info(
                        "RQ2 target-specific progress %s/%s | target=%s feature_set=%s model=%s | elapsed=%s eta=%s",
                        progress.completed, progress.total, target, fs_name, model_name, format_seconds(elapsed), format_seconds(eta)
                    )
    else:
        for target, fs, spec in tasks:
            target_name, fs_name, model_name, row = _run_target_specific_task(
                target,
                fs,
                spec,
                x_user,
            ...
            if logger is not None:
                logger.info(
                    "RQ2 target-specific progress %s/%s | target=%s feature_set=%s model=%s | elapsed=%s eta=%s",
                    progress.completed, progress.total, target_name, fs_name, model_name, format_seconds(elapsed), format_seconds(eta)
                )

    return pd.DataFrame(rows).sort_values(by=["target", "r2"], ascending=[True, False]).reset_index(drop=True)
