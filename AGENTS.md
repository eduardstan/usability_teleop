...
Every stage command writes:
- immutable history record: `outputs/runs/run_manifest_<command>_<timestamp>.json`
- moving pointer: `outputs/runs/run_manifest_<command>_latest.json`

Manifest payload includes command args, start/end UTC, elapsed seconds, git commit, status, outputs, and error details.
